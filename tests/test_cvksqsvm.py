...
import numpy
import time
from torchsvm.cvksqsvm import cvksqsvm
...
class Testcvksqsvm(unittest.TestCase):
    def test_fit_predict(self):
        ...
        model1 = cvksqsvm(Kmat=Kmat, y=y_train, nlam=nlam, ulam=ulam, foldid=foldid, nfolds=nfolds, eps=1e-5, maxit=1000, gamma=1e-8, device='cuda')
        model1.fit()
